...
model = dict(
    ...
    pretrained='path to pretrained_models/EgoWholeMocap_limb_pretrain.pth',
    backbone=dict(
        type='UndistortViT',
...
train_pipeline = [
    ...
    dict(type='ToTensor'),
    #dict(type='ToTensor', device='cuda:1'),
    dict(
...
data = dict(
    ...
    train=dict(
        type='LimbRenderpeopleMixamoDataset',
        ann_file='path to dataset_dir/renderpeople_mixamo_labels_old.pkl',   
        img_prefix='path to dataset_dir',                                    
        data_cfg=data_cfg,
        pipeline=train_pipeline,
        test_mode=False,
    ),
    val=dict(
        type='LimbMocapStudioDataset',
        data_cfg=data_cfg,
