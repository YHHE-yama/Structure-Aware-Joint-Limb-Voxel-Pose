...
class CrossTGFI3D(nn.Module):
    ...
    def _make_attn_mask(self, pj, pl, joint_to_limb_mask):
        ...
        pos_bias = -F.softplus(self.dist_scale) * dist2    
...
class TopKCrossattenyionHeatmap3DNet_v3(nn.Module):
    def __init__(self,
        ...
        self.limb_head = nn.Sequential(
            nn.Conv3d(joint_num, 64, kernel_size=3, padding=1),  
            nn.BatchNorm3d(64),
            nn.ReLU(inplace=True),
            nn.Conv3d(64, 64, kernel_size=3, padding=1),
            nn.BatchNorm3d(64),
            nn.ReLU(inplace=True),
            nn.Conv3d(64, len(LIMB_CONNECTIONS), kernel_size=1)  
                )
