...
class CrossTGFI3D(nn.Module):
    """
    第二版：支持禁用 cross-attention 的位置偏置与相邻掩码。
    当 use_pos_bias=False 或 force_no_mask=True 时，不向 MHA 提供任何 attn_mask（即无 bias、无 mask）。
    其它逻辑（Top-K、写回、扩散、双向开关）保持不变。
    """
    def __init__(self, d_model=64, nhead=4, k_joint=32, k_limb=64,
                 bidirectional=True, restrict_to_adjacent=True,
                 diffusion_radius=1,
                 use_pos_bias=False,          # <--- 新增：是否使用基于距离的加性位置偏置
                 force_no_mask=True):       # <--- 新增：强制不使用任何 mask（覆盖 restrict_to_adjacent）
        super().__init__()
        ...
        # 仅当使用位置偏置时才会用到
        self.dist_scale = nn.Parameter(torch.tensor(2.0))  # softplus 保证非负
        self._dbg_counter = 0
    ...
    def _topk_tokenize_3d(x, k):
        ...
        N = D * H * W
        flat = x.view(B, C, N)
        val, idx = torch.topk(flat, min(k, N), dim=-1)    # [B,C,K]
    ...
    def _make_attn_mask(self, pj, pl, joint_to_limb_mask):
        """
        构造基于距离的加性偏置 +（可选）相邻掩码。
        返回：attn_mask 形状 [B, J*K, L*K]，dtype=float32。
        若 self.use_pos_bias=False 或 self.force_no_mask=True，则返回 None。
        """
        if self.force_no_mask or (not self.use_pos_bias):
            #print('no mask!!!!!!!!!!!!!!!!!!!')
            return None  # 无 bias、无 mask

        B, J, Kj, _ = pj.shape
        L, Kl = pl.shape[1], pl.shape[2]

        Pj = pj.view(B, J*Kj, 3)
        Pl = pl.view(B, L*Kl, 3)
        dist2 = self._pairwise_dist(Pj, Pl)                # [B, J*K, L*K]
        pos_bias = -F.softplus(self.dist_scale) * dist2    # 非负尺度

        if self.restrict and (joint_to_limb_mask is not None):
            # 构造相邻约束 mask
            m = joint_to_limb_mask[None, :, None, :, None].to(torch.bool).to(Pj.device)
            m = m.expand(B, J, Kj, L, Kl).reshape(B, J*Kj, L*Kl)

            # 兜底：保证每个 query 至少有一个 key
            row_all_masked = ~m.any(dim=-1)  # [B, J*K]
            if row_all_masked.any():
                nearest = dist2.argmin(dim=-1)  # [B, J*K]
    ...
    def forward(self, joint_hm, limb_hm, joint_to_limb_mask=None):
        ...
        joint_hm: [B, J, D,H,W]
        limb_hm:  [B, L, D,H,W]
        joint_to_limb_mask: [J, L] (bool) 可选
        """
        B, J, D, H, W = joint_hm.shape
        L = limb_hm.shape[1]

        vj, ij, pj = self._topk_tokenize_3d(joint_hm, self.kj)  # [B,J,K,1], [B,J,K], [B,J,K,3]
        vl, il, pl = self._topk_tokenize_3d(limb_hm,  self.kl)  # [B,L,K,1], [B,L,K], [B,L,K,3]
        ...
        # —— 关键分支：是否提供 attn_mask（含位置偏置/相邻掩码）——
        attn_mask_fullbatch = self._make_attn_mask(pj, pl, joint_to_limb_mask)

        if attn_mask_fullbatch is None:
            # 无 bias、无 mask：可一次性 batch 计算
            out_j, _ = self.attn_jq_lkv(tj, tl, tl, attn_mask=None, key_padding_mask=None, need_weights=False)
        else:
            # 每个 batch 独立的 mask
            outs = []
            for b in range(B):
                out_b, _ = self.attn_jq_lkv(
                    ...
                    attn_mask=attn_mask_fullbatch[b],
        ...
        # （可选）双向 cross（与上面同样的 mask 策略）
        if self.bidir:
            if attn_mask_fullbatch is None:
                out_l, _ = self.attn_lq_jkv(tl, tj, tj, attn_mask=None, key_padding_mask=None, need_weights=False)
            else:
                outs = []
                for b in range(B):
                    out_b, _ = self.attn_lq_jkv(
                        tl[b:b+1], tj[b:b+1], tj[b:b+1],
                        attn_mask=attn_mask_fullbatch[b].transpose(1, 2),
                        key_padding_mask=None, need_weights=False
                    )
                    outs.append(out_b)
                out_l = torch.cat(outs, dim=0)
            tl = tl + out_l
        ...
        # 若需要扩散，可在 _scatter_writeback 里替代上面的写回实现
        if self.diffusion_radius and self.diffusion_radius > 0:
            # 以“增量”形式扩散：用写回后的 fused_joint 覆盖 joint_hm
            # 如果你希望“叠加增量再扩散”，可把 delta = v_new - vj，走 _scatter_writeback(delta, ...) 再加回去
            pass
...
class TopKCrossattenyionHeatmap3DNet_v3(nn.Module):
    def __init__(self,
                 in_channels=768,
                 num_deconv_layers=2,
        ...
        super(TopKCrossattenyionHeatmap3DNet_v3, self).__init__()
        ...
        self.cross_tgfi = CrossTGFI3D(
            ...
            use_pos_bias=False,           # 关闭位置偏置（不再传加性 bias 到 MHA）
            force_no_mask=True           # 或者 True 来强制不传 mask（即便 restrict=True）
        )
        # 为相邻约束准备关节-肢体邻接掩码
        J = self.joint_num
